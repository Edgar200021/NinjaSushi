...
import styles from './Sushi.module.sass'
...
const Sushi = () => {
  ...
  const fetchData = async url => {
    ...
    setData(data.filter(item => item.category === 'sushi'))
    setFetchError(false)
    setLoading(false)
  }
  ...
  const emptyContent = !data.length ? (
    <>
      <h3 className='empty-title'>Товаров пока нет</h3>
      <img className="empty-img" src={emptyImg} alt="пустая коробка" />
    </>
  ) : null
  const error = fetchError ? (
    <>
      <h3 className="error-title">Произошла ошибка</h3>
      <img className="error-img" src={errorImg} alt="Ошибка" />
    </>
  ) : null

  return (
    <main className={styles.main}>
      <h2 className={cn(styles.main__title, 'second-title')}>Суши</h2>
      {loading ? (
        <Spinner />
      ) : (
        error ||
